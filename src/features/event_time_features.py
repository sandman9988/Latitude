...
FULL_DAY_MINUTES: Final[int] = 24 * 60
...
class EventTimeFeatureEngine:
    ...
    def calculate_features(self, dt: datetime | None = None) -> dict[str, float]:
        ...
        if len(self.cache) > CACHE_MAX_ENTRIES:
            ...
            oldest_keys = list(self.cache.keys())[:CACHE_PRUNE_BATCH]
    ...
    def _calc_session_proximity(self, dt: datetime, event_minutes: int) -> tuple[float, float]:
        ...
        # Handle day wrap
        if diff < -HALF_DAY_MINUTES:  # More than 12 hours in past = upcoming tomorrow
            diff += FULL_DAY_MINUTES
        elif diff > HALF_DAY_MINUTES:  # More than 12 hours in future = was yesterday
            diff -= FULL_DAY_MINUTES
    ...
    def is_high_liquidity_period(self, dt: datetime | None = None) -> bool:
        ...
        if features["london_is_active"] > 0:
            return True
        return features["new_york_is_active"] > 0
...
def main() -> None:
    ...
    test_times = [
        ...
        datetime(2026, 1, 10, 12, 0, tzinfo=UTC),  # NY open
        datetime(2026, 1, 10, 22, 0, tzinfo=UTC),  # Rollover
        datetime(2026, 1, 11, 3, 0, tzinfo=UTC),  # Asian session
    ]
    ...
    wed_mid = datetime(2026, 1, 14, 12, 0, tzinfo=UTC)
