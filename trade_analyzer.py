...
class TradeAnalyzer:
    ...
    def _prepare_data(self):
        ...
        numeric_cols = ['pnl', 'mfe', 'mae', 'entry_price', 'exit_price', 
                       'capture_efficiency', 'equity_after', 'duration_seconds']
    ...
    def analyze_dual_agents(self) -> Dict:
        ...
        return {
            ...
            'capture_by_harvester_quality': capture_by_harvester.to_dict() if capture_by_harvester is not None else None
        }
    ...
    def analyze_capture_efficiency(self) -> Dict:
        ...
        return {
            ...
            'pct_above_50': (self.df['capture_efficiency'] > 0.5).sum() / len(self.df) * 100
        }
    ...
    def print_report(self):
        ...
        for hour, row in hourly.iterrows():
            print(f"{hour:02d}:00      ${row['total_pnl']:<13.2f} ${row['avg_pnl']:<13.2f} {int(row['num_trades']):<10} {row['win_rate']*100:.1f}%")
        ...
        for day, row in daily.iterrows():
            print(f"{day:<15} ${row['total_pnl']:<13.2f} ${row['avg_pnl']:<13.2f} {int(row['num_trades']):<10} {row['win_rate']*100:.1f}%")
        ...
        for _, trade in best.iterrows():
            print(f"{int(trade['trade_num']):<10} {str(trade['entry_time']):<20} {trade['direction']:<8} "
