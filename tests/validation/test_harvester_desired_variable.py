...
class TestHarvesterDesiredVariable:
    ...
    def test_desired_is_zero_on_close_signal(self, xauusd_m5_bars_100):
    ...
    def test_desired_is_cur_pos_on_hold_signal(self, xauusd_m5_bars_100):
    ...
    def test_desired_never_none_for_any_exit_action(self):
        ...
        for cur_pos in [-1, 1]:
            for exit_action in [0, 1]:
                desired = _compute_desired(exit_action, cur_pos)
                assert desired is not None, f"desired=None for exit_action={exit_action}, cur_pos={cur_pos}"
    ...
    def test_harvester_sl_triggers_close_at_high_mae(self, xauusd_m5_bars_100):
        """
        ...
        assert exit_action == 1, f"Expected CLOSE (exit_action=1) after large MAE, got {exit_action} conf={conf}"
