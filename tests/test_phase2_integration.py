...
try:
    ...
    if SafeMath.is_close(prev, 103.0):
...
try:
    ...
    if monitor.is_stagnant:
        ...
        print("    ✗ FAIL: Should detect stagnation after 15 bars")
        sys.exit(1)
    ...
    if monitor.bars_since_trade == 0:
        print("    ✓ Trade resets inactivity counter")
    else:
...
try:
    ...
    # Calculate reward
    reward_data = {"exit_pnl": 50.0, "mfe": 100.0, "mae": -20.0, "winner_to_loser": False}
    ...
    if all(key in rewards for key in required_keys):
        print("    ✓ All 5 reward components present")
...
try:
    ...
    if hasattr(policy, "update_ensemble_weights"):
        print("    ✓ Policy has update_ensemble_weights method")
    else:
        print("    ✗ FAIL: Missing update_ensemble_weights method")
        sys.exit(1)
    ...
    if hasattr(policy, "get_ensemble_stats"):
        ...
        print("    ✗ FAIL: Missing get_ensemble_stats method")
        sys.exit(1)
