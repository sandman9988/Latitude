...
class TriggerAgent:
    ...
    def decide(
        ...
        if self.paper_mode or self.disable_gates:
            ...
            if random.random() < self.epsilon:
                # Random action: 0=FLAT, 1=LONG, 2=SHORT
                action = random.choice([0, 1, 2])
                LOG.info("[TRIGGER] EXPLORE: random action=%d (ε=%.3f, bars_flat=%d)", 
                         action, self.epsilon, self.bars_since_trade)
                self._decay_epsilon()
                self.bars_since_trade = 0  # Reset counter when taking action
                return action, 0.5, 0.0015
        ...
        if not self.use_torch:
            ...
            predicted_runway = 0.0015  # Conservative estimate: 15 pips
            ...
            if not self.paper_mode and confidence < self.confidence_floor:
                LOG.debug("[TRIGGER] BLOCKED by confidence floor: %.3f < %.3f",
        ...
        with self.torch.no_grad():
            ...
            if not self.paper_mode and calibrated_prob < self.confidence_floor:
                LOG.debug(
                    ...
                    calibrated_prob, self.confidence_floor
                )
            ...
            LOG.debug(
                ...
                q_values, action, raw_prob, calibrated_prob, breakeven_prob, predicted_runway
            )
    ...
    def _fallback_strategy(self, state: np.ndarray, regime_threshold_adj: float = 0.0) -> int:
        ...
        Paper mode: Adds epsilon-greedy exploration to force some trades.
        ...
        if state.shape[0] == 0 or state.shape[1] < 3:
            return 0  # NO_ENTRY
        # Paper mode: Epsilon-greedy exploration to force trades for learning
        paper_mode = os.environ.get('PAPER_MODE') == '1'
        epsilon = 0.15 if paper_mode else 0.0  # 15% random exploration in paper mode
        
        if paper_mode and random.random() < epsilon:
            # Random entry for exploration
            action = random.choice([1, 2])  # LONG or SHORT
            LOG.info("[TRIGGER] PAPER MODE: Epsilon exploration → action=%d", action)
            return action
        
        # Get latest features (already normalized)
        ma_diff = float(state[-1, 2])  # MA fast/slow difference
        imbalance = float(state[-1, 4]) if state.shape[1] > 4 else 0.0
        ...
        base_threshold = 0.15 if paper_mode else 0.3
    ...
    def update_platt_params(self, predicted_prob: float, actual_outcome: float):
        ...
        LOG.debug(
            ...
            self.platt_a, self.platt_b, predicted_prob, actual_outcome
        )
    ...
    def _q_to_runway(self, q_value: float) -> float:
        ...
        if q_value <= 0:
            ...
            return 0.0050  # 50 pips maximum
        else:
            # Linear interpolation: Q in [0, 3] → Runway in [0.001, 0.005]
            return 0.0010 + (q_value / 3.0) * 0.0040
    ...
    def update_from_trade(self, actual_mfe: float, predicted_runway: float):
        ...
        if predicted_runway > 0:
            ...
            LOG.debug(
                ...
                predicted_runway, actual_mfe, error_pct
            )
...
if __name__ == "__main__":
    ...
    assert trigger.use_torch == False
    ...
    assert trigger._q_to_runway(0.0) == 0.0010
    assert trigger._q_to_runway(1.5) == 0.0030
    assert trigger._q_to_runway(3.0) == 0.0050
