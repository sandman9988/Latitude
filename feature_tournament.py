...
# Feature stability constants
MIN_REGIME_SAMPLES: int = 10  # Minimum samples needed per regime
...
class FeatureTournament:
    ...
    def run_tournament(self) -> dict:
        ...
        for i in range(self.n_features):
            ...
            # Combined score
            new_scores[i] = 0.5 * predictive_power + 0.3 * regime_stability + 0.2 * diversity
        ...
        active_names = [self.feature_names[i] for i in range(self.n_features) if self.feature_active[i]]
        ...
        result = {
            ...
            "top_5_features": [self.feature_names[i] for i in np.argsort(self.feature_scores)[-5:][::-1]],
        }

        if self.tournaments_run % 10 == 0:
            LOG.info(
                "[FEATURE-TOURNAMENT] Round %d: %d/%d active, top: %s",
                self.tournaments_run,
                result["n_active"],
                self.n_features,
    ...
    def _calculate_regime_stability(self, feature: np.ndarray, target: np.ndarray, regimes: np.ndarray) -> float:
        """
        ...
        for regime in unique_regimes:
            mask = regimes == regime
            if np.sum(mask) < MIN_REGIME_SAMPLES:  # Need minimum samples
                continue
        ...
        if len(regime_corrs) == 0:
            return DEFAULT_STABILITY  # Default middle value

        # Stability = 1 - coefficient of variation
        mean_corr = np.mean(regime_corrs)
        std_corr = np.std(regime_corrs)

        if SafeMath.is_zero(mean_corr):
            return 0.0
