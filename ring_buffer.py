...
class RollingVariance:
    ...
    def __repr__(self) -> str:
        return (f"RollingVariance(period={self.period}, mean={self.mean:.6f}, "
...
class RollingStats:
    ...
    def is_ready(self) -> bool:
        ...
        return (self.mean_tracker.is_ready() and 
                self.var_tracker.is_ready() and 
                self.minmax_tracker.is_ready())
        
    def __repr__(self) -> str:
        return (f"RollingStats(period={self.period}, mean={self.mean:.6f}, "
...
if __name__ == "__main__":
    ...
    # Mean should be 40.0
    expected_mean = 40.0
    if abs(mean_tracker.value - expected_mean) < 1e-9:
    ...
    if mean_error < 1e-9 and std_error < 1e-9:
    ...
    if abs(minmax_tracker.min_value - expected_min) < 1e-9 and abs(minmax_tracker.max_value - expected_max) < 1e-9:
        ...
        print(f"    ERROR: Min {minmax_tracker.min_value} vs {expected_min}, Max {minmax_tracker.max_value} vs {expected_max}")
        exit(1)
    ...
    if speedup < 1.5:
        print(f"    WARNING: Expected >1.5x speedup, got {speedup:.2f}x")
