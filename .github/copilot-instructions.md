...
Active paper trading XAUUSD M5, Pepperstone demo. Python 3.12, 39 713 production lines, 2 595 tests passing.
...
| File | Purpose |
|------|---------|
| `src/core/ctrader_ddqn_paper.py` | Main bot orchestrator (6 354 lines) |
| `src/agents/trigger_agent.py` | Entry DDQN + fallback strategy (830 lines) |
| `src/agents/harvester_agent.py` | Exit DDQN + min-hold guard (906 lines) |
...
| `src/monitoring/hud_tabbed.py` | 7-tab curses HUD (3 855 lines) |
...
- Run `python -m pytest tests/ -q` before committing — must stay at 2 595 passing
...
## Current open items (as of 2026-03-08)
