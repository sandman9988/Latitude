...
class AgentArena:
    ...
    def __init__(
        ...
        LOG.info(
            ...
            len(trigger_agents), len(harvester_agents), consensus_mode.value
        )
    ...
    def get_entry_signal(
        ...
        for i, agent in enumerate(self.trigger_agents):
            try:
                action, confidence, runway = agent.decide(
                    bars, imbalance, vpin_z, depth_ratio, realized_vol
                )
        ...
        LOG.debug(
            ...
            actions, consensus_action, consensus_confidence, agreement
        )
    ...
    def get_exit_signal(
        ...
        for i, agent in enumerate(self.harvester_agents):
            try:
                action, confidence = agent.decide(
                    bars, current_price, imbalance, vpin_z, depth_ratio
                )
                actions.append(action)
                confidences.append(confidence)
            except Exception as e:
    ...
    def _voting_consensus(
        ...
        winning_confidences = [
            conf for act, conf in zip(actions, confidences) if act == consensus_action
        ]
    ...
    def _min_risk_consensus(
        ...
        for action, stats in action_votes.items():
            ...
            if vote_ratio >= self.min_agreement and avg_confidence >= 0.6:
        ...
        return no_action, 0.5, agreement
    ...
    def get_diversity_score(self) -> Dict[str, float]:
        ...
        return {
            ...
            'harvester_diversity': harvester_diversity
        }
    ...
    def train_all_agents(self, batch_size: int = 64) -> Dict[str, List[float]]:
        ...
        return {
            ...
            'harvester_losses': harvester_losses
        }
