...
from safe_utils import SafeMath, safe_percentile, safe_std

MIN_KURTOSIS_SAMPLE = 30
MIN_KURTOSIS_STATS = 4
STD_EPS = 1e-12
MIN_VAR_SAMPLE = 30
MIN_BASE_VAR_SAMPLE = 10
VPIN_CAP = 5.0
KURTOSIS_CAP = 50.0
VAR_MULT_MAX = 10.0
REFERENCE_VOL_FALLBACK = 0.01
MIN_VAR_THRESHOLD = 1e-12
...
class KurtosisMonitor:
    ...
    def update(self, return_value: float) -> tuple[float, bool]:
        ...
        if len(self.returns) < MIN_KURTOSIS_SAMPLE:  # Need minimum sample
            return 0.0, False
    ...
    def _calculate_kurtosis(self) -> float:
        ...
        if len(returns_array) < MIN_KURTOSIS_STATS:
            return 0.0
...
class VaREstimator:
    ...
    def estimate_var(
        ...
        vpin_z_capped = max(-VPIN_CAP, min(VPIN_CAP, vpin_z if SafeMath.is_valid(vpin_z) else 0.0))
        ...
        kurtosis_capped = max(0.0, min(KURTOSIS_CAP, kurtosis if SafeMath.is_valid(kurtosis) else 0.0))
        ...
        max_var = base_var * VAR_MULT_MAX  # Never more than 10x base
        var = min(var, max_var)
    ...
    def _calculate_vol_mult(self, current_vol: float | None) -> float:
        ...
        if self._reference_vol < STD_EPS:
            self._reference_vol = REFERENCE_VOL_FALLBACK
