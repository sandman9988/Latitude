...
def test_trigger_agent_training():
    ...
    try:
        ...
        for _ in range(BUFFER_SMALL):
            agent.add_experience(
                state=state + rng.standard_normal((64, 7)) * 0.1,
                action=rng.integers(0, 3),  # 0=NO_ENTRY, 1=LONG, 2=SHORT
                reward=rng.standard_normal() * 0.5,
                next_state=next_state + rng.standard_normal((64, 7)) * 0.1,
                done=True,
            )
...
def test_harvester_agent_training():
    ...
    try:
        ...
        for _ in range(BUFFER_SMALL):
            agent.add_experience(
                state=state + rng.standard_normal((64, 10)) * 0.1,
                action=rng.integers(0, 2),  # 0=HOLD, 1=CLOSE
                reward=rng.standard_normal() * 0.5,
                next_state=next_state + rng.standard_normal((64, 10)) * 0.1,
                done=True,
            )
...
def test_experience_buffer():
    ...
    try:
        ...
        LOG.info("✓ Buffer initialized with capacity 1000")

        # Add experiences
        LOG.info("Adding 200 experiences...")
        for _ in range(BUFFER_MEDIUM):
            state = rng.standard_normal((64, 7)).astype(np.float32)
            next_state = rng.standard_normal((64, 7)).astype(np.float32)
            ...
            buffer.add(
                ...
                done=rng.random() > DONE_THRESHOLD,
        ...
        if batch:
            LOG.info("✓ Sampled batch successfully")
