...
def test_trigger_agent_training():
    ...
    assert trigger.buffer.capacity == 50_000
    assert trigger.enable_training == True
    ...
    for i in range(100):
        entry_state = np.random.randn(64, 7).astype(np.float32)
        exit_state = np.random.randn(64, 7).astype(np.float32)
        action = np.random.choice([1, 2])  # LONG or SHORT
        
        # Runway utilization reward: actual_MFE / predicted_runway
        predicted_runway = 0.002  # 20 pips
        actual_mfe = np.random.uniform(0.0005, 0.0035)  # 5-35 pips
    ...
    assert stats['ready_to_train'] == False  # Need 1000 minimum
    ...
    for i in range(950):
        ...
        reward = np.random.uniform(-1.0, 2.0)
    ...
    print(f"✓ Buffer size: {stats['buffer_size']}")
    print(f"  Ready to train: {stats['ready_to_train']}")
    assert stats['ready_to_train'] == True
...
def test_harvester_agent_training():
    ...
    assert harvester.buffer.capacity == 50_000
    assert harvester.enable_training == True
    ...
    for i in range(1200):
        ...
        action = np.random.choice([0, 1])  # HOLD or CLOSE
        
        # Capture efficiency reward: exit_pnl / MFE
        # Good exits: high capture ratio
        # WTL: negative reward
        capture_ratio = np.random.uniform(0.0, 1.2)
...
def test_defensive_validation():
    ...
    trigger.add_experience(
        ...
        action=1,
        reward=1.0,
        next_state=np.random.randn(64, 7),
        done=True
    )
    
    # Non-finite reward
    trigger.add_experience(
        state=np.random.randn(64, 7),
        action=1,
        reward=np.nan,
        next_state=np.random.randn(64, 7),
        done=True
