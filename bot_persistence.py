...
class BotPersistenceManager:
    ...
    def save_agent_model(
        ...
        try:
            ...
            self.persistence.save_json(
                metadata,
                str(meta_file.relative_to(self.base_dir))
            )
            
            LOG.info(
                f"[PERSISTENCE] Saved {agent_type} agent {agent_idx} model: "
                f"{symbol}/{timeframe}"
            )
            return True
    ...
    def load_agent_model(
        ...
        if not model_file.exists():
            LOG.warning(
                f"[PERSISTENCE] Model not found: {agent_type} {agent_idx} "
        ...
        try:
            ...
            LOG.info(
                f"[PERSISTENCE] Loaded {agent_type} agent {agent_idx}: "
                f"{symbol}/{timeframe}"
            )
            return state_dict
        except Exception as e:
    ...
    def save_session_stats(
        ...
        success = self.persistence.save_json(
            stats,
            str((stats_dir / session_file).relative_to(self.base_dir))
        )
        
        if success:
            LOG.info(
                f"[PERSISTENCE] Saved session stats: {symbol}/{timeframe}/{session_id}"
            )
    ...
    def save_cumulative_stats(
        ...
        existing = self.persistence.load_json(
            str(cumulative_file.relative_to(self.base_dir))
        )
        ...
        if success:
            LOG.info(
                f"[PERSISTENCE] Updated cumulative stats: {symbol}/{timeframe}"
            )
    ...
    def load_cumulative_stats(
        ...
        return self.persistence.load_json(
            str(cumulative_file.relative_to(self.base_dir))
        )
    ...
    def save_global_stats(
        ...
        self.persistence.save_json(
            ...
            "cumulative/all_instruments.json"
        )
        
        self.persistence.save_json(
            {'timeframes': list(by_timeframe.values())},
            "cumulative/all_timeframes.json"
        )
        
        self.persistence.save_json(
            total_stats,
            "cumulative/all_sessions.json"
        )
    ...
    def save_checkpoint(
        ...
        success = self.persistence.save_json(
            state,
            str(checkpoint_file.relative_to(self.base_dir))
        )
        
        if success:
            LOG.info(
                f"[PERSISTENCE] Saved checkpoint '{checkpoint_name}': "
                f"{symbol}/{timeframe}"
            )
    ...
    def load_checkpoint(
        ...
        if not checkpoint_file.exists():
            LOG.warning(
                f"[PERSISTENCE] Checkpoint not found: {checkpoint_name} "
                f"for {symbol}/{timeframe}"
            )
            return None
    ...
    def list_saved_models(self, symbol: str, timeframe: str) -> Dict[str, List[int]]:
        ...
        return {
            'trigger': sorted(trigger_models),
            'harvester': sorted(harvester_models)
        }
